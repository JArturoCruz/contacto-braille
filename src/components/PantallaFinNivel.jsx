...
export default function PantallaFinNivel({
  ...
  // CORRECCIÓN: La precisión se calcula sobre los INTENTOS, no sobre el total de preguntas.
  ...
  return (
    <div style={{ background: 'linear-gradient(135deg, #1e1b4b 0%, #2d1b69 50%, #1e1b4b 100%)', minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', padding:24, position:'relative', overflow:'hidden' }}>
      ...
      <div style={{ maxWidth:500, width:'100%', textAlign:'center', position:'relative', zIndex:1 }}>
        ...
        <div style={{
          background:'rgba(255,255,255,0.08)', backdropFilter:'blur(10px)', borderRadius:20, padding:28, marginBottom:20,
          border:'1px solid rgba(255,255,255,0.12)', opacity: step>=2?1:0, transform: step>=2?'translateY(0)':'translateY(20px)',
          ...
          <div style={{ marginTop:20, textAlign:'left' }}>
            ...
            <div style={{ height:8, background:'rgba(255,255,255,0.1)', borderRadius:99, overflow:'hidden' }}>
              <div style={{
                height:'100%', width: `${Math.min(100, (xpTotal % 500) / 500 * 100)}%`,
                background:'linear-gradient(90deg, #7c3aed, #f59e0b)', borderRadius:99, transition:'width 1s ease 0.8s',
              }}/>
            </div>
          </div>
        </div>
        ...
        {logroNuevo && mostrarLogro && (
          <div style={{
            background: `linear-gradient(135deg, ${logroNuevo.color}22, rgba(255,255,255,0.06))`, border: `2px solid ${logroNuevo.color}`,
            borderRadius:16, padding:'18px 24px', marginBottom:20, textAlign:'left', animation:'slideIn 0.6s cubic-bezier(0.34,1.56,0.64,1)',
          }}>
        ...
        <div style={{
          display:'flex', flexDirection:'column', gap:12, opacity: step>=4?1:0, transform: step>=4?'translateY(0)':'translateY(20px)',
          ...
          {onSiguiente && (
            <button
              ...
              onClick={onSiguiente} onMouseEnter={e => e.target.style.transform='scale(1.03)'} onMouseLeave={e => e.target.style.transform='scale(1)'}>
              ▶ Siguiente Nivel
            </button>
          )}
