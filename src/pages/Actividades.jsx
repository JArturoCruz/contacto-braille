...
export default function Actividades() {
  ...
  const tiempoRef = useRef(nivel?.tiempoLimite || 0)
  ...
  }, [indiceActual, juegoIniciado, secuencia])
  ...
  // CORRECCIÓN: Evitar matriz vacía cuando el modo es "leer"
  useEffect(() => {
    if (!pregunta) return;
    ...
    if (pregunta.tipo === 'completar') {
      ...
      setMatriz(nueva)
    } else if (pregunta.tipo === 'leer') {
      setPuntosFijos([])
      // Mostramos la matriz correcta al usuario para que la pueda leer
      setMatriz([...correcta])
    } else {
      setPuntosFijos([])
    ...
    setEstado('jugando')
  }, [indiceActual, secuencia, pregunta, correcta, nivel, puntosPreMarcados])

  const opciones = useCallback(() => {
    if (!pregunta || pregunta.tipo !== 'leer') return []
  ...
  }, [indiceActual, secuencia, pregunta, opciones])

  const finalizarNivel = useCallback(() => {
    const a = aciertosRef.current
    const i = intentosRef.current
    const t = tiempoRef.current
    
    const precision = i > 0 ? Math.round((a / i) * 100) : 100
    const estrellas = calcularEstrellas(precision, nivel)
    const xp = nivel.xpPorAcierto * a

    const bonusTiempo = nivel.tiempoBonus && t > 0 ? Math.round(t * 0.5) : 0
  ...
  const validar = useCallback(() => {
    ...
    if (pregunta.tipo === 'leer') {
      // CORRECCIÓN: Obligamos a elegir una opción antes de gastar intentos
      if (!opcionSeleccionada) {
        hablar('Por favor, selecciona una opción antes de validar')
    ...
    intentosRef.current = nuevosIntentos
  ...
  }, [estado, pregunta, matriz, correcta, opcionSeleccionada, indiceActual, secuencia, puntosFijos, intentos, aciertos, finalizarNivel])

  const limpiar = useCallback(() => {
    if (estado !== 'jugando' || !pregunta) return
    if (pregunta.tipo === 'leer') return // En modo lectura no tiene caso limpiar matriz
    const nueva = [0,0,0,0,0,0]
    puntosFijos.forEach(i => { nueva[i] = 1 })
    setMatriz(nueva)
    setEstado('jugando')
  ...
  }, [puntosFijos, nivel, estado, pregunta])
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (e.key === 'Escape' && esEscribir) limpiar()
    }
  ...
  const togglePunto = (indice) => {
    if (estado !== 'jugando' || !pregunta || pregunta.tipo === 'leer') return
    if (puntosFijos.includes(indice)) return
    const nueva = [...matriz]
    nueva[indice] = nueva[indice] === 1 ? 0 : 1
    setMatriz(nueva)
    ...
    return (
      <PantallaFinNivel
        ...
        onReintentar={() => {
          ...
          setEstado('jugando')
          const initTime = nivel.tiempoLimite || 0
          setTiempo(initTime)
          tiempoRef.current = initTime
        }}
    ...
    return (
      <div style={si.page}>
        <div style={si.header}>
          ...
          <span style={si.headerTitulo} tabIndex={0} onFocus={() => hablar(`Nivel ${nivel.id}: ${nivel.titulo}`)}>Nv.{nivel.id} {nivel.titulo}</span>
          <span style={si.headerContador} tabIndex={0} onFocus={() => hablar(`Pregunta ${indiceActual + 1} de ${secuencia.length}`)}>{indiceActual + 1}/{secuencia.length}</span>
        </div>
        <div style={si.progressBar}><div style={{ ...si.progressFill, width:`${progreso}%` }}/></div>
        
        {nivel.tiempoBonus && (
          <div style={{ background: tiempo < 20 ? '#7f1d1d' : '#2e2a5e', borderRadius:8, padding:'8px 12px', textAlign:'center', color: tiempo < 20 ? '#fca5a5' : '#c4b5fd', fontWeight:800, fontSize:14 }} tabIndex={0} onFocus={() => hablar(`Tiempo restante: ${tiempo} segundos`)}>⏱ {tiempo}s</div>
        )}

        <div style={si.letraCard} tabIndex={0} onFocus={() => hablar(esEscribir ? `Escribe la letra ${pregunta.letra} en braille` : `¿Qué letra es esta matriz?`)} onMouseEnter={() => hablar(esEscribir ? `Escribe la letra ${pregunta.letra}` : `Identifica la letra`)}>
          {esEscribir ? (
            <><span style={si.letraGrande}>{pregunta.letra}</span><span style={si.letraDesc}>{pregunta.tipo === 'completar' ? 'Completa la matriz' : 'Escribe en braille'}</span></>
          ) : (
            <><span style={{ color:'#c4b5fd', fontSize:15 }}>¿Qué letra es?</span><span style={{ color:'#a78bfa', fontSize:12 }}>Elige la correcta</span></>
          )}
        </div>

        <div style={{ display:'flex', justifyContent:'center', alignItems:'center', gap:20, padding:8 }}>
          <MatrizBraille matriz={matriz} correcta={correcta} estado={estado} onToggle={esEscribir ? togglePunto : null} teclaPulsada={teclaPulsada} mostrarPista={mostrarPista} invidente={true} />
              ...
              <div style={{ display:'grid', gridTemplateColumns:'32px 32px', gap:5 }}>
                {LAYOUT.map(({ numero, tecla, indice }) => (
                  <div key={indice} style={{
                    ...
                    color: teclaPulsada === tecla.toLowerCase() ? 'white' : '#c4b5fd', fontWeight:700, fontSize:12,
                  }}>
                    <span>{tecla}</span><span style={{ fontSize:8, opacity:0.7 }}>P{numero}</span>
                  </div>
                ))}
              </div>
              <div style={{ fontSize:9, color:'#7c6fad', marginTop:6, textAlign:'center', lineHeight:1.6 }}>Enter=validar<br/>Esc=limpiar</div>
            </div>
          )}
        </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
            {opcionesActuales.map(op => (
              <button key={op} style={{
                ...
                }} onClick={() => { setOpcionSeleccionada(op); hablar(`Opción seleccionada: ${op}`) }} tabIndex={0} onFocus={() => hablar(`Opción ${op}`)}>
                {op}
              </button>
            ))}
          </div>
          ...
          {esEscribir && (
            <>
              ...
              {!nivel.pistaAutomatica && (
                <button style={{ ...si.btnSec, background: mostrarPista ? '#7c3aed' : '#2e2a5e' }} onClick={() => { setMostrarPista(!mostrarPista); hablar(mostrarPista ? 'Pista ocultada' : 'Pista activada') }} onFocus={() => hablar('Pista')}>💡</button>
              )}
            </>
          )}
          <button style={si.btnPrimario} onClick={validar} onFocus={() => hablar('Validar, o presiona Enter')}>✓ Validar</button>
        </div>

        <div style={si.statsRow}>
          {[{ v:aciertos, l:'Aciertos', c:'#22c55e', desc:`Aciertos: ${aciertos}` }, { v:intentos, l:'Intentos', c:'#ea580c', desc:`Intentos: ${intentos}` }, { v:secuencia.length - indiceActual, l:'Restantes', c:'#a78bfa', desc:`Restantes: ${secuencia.length - indiceActual}` }].map((s,i) => (
            <div key={i} style={si.statItem} tabIndex={0} onFocus={() => hablar(s.desc)}><span style={{ fontSize:18, fontWeight:800, color:s.c }}>{s.v}</span><span style={{ fontSize:11, color:'#c4b5fd' }}>{s.l}</span></div>
          ))}
        </div>
      </div>
    )
  ...
  return (
    <div style={sv.page}>
      ...
      <div style={{ display:'flex', gap:12, alignItems:'center', marginBottom:16 }}>
        <div style={{ flex:1, height:10, background:'#e5e7eb', borderRadius:99, overflow:'hidden' }}><div style={{ height:'100%', width:`${progreso}%`, background:'linear-gradient(90deg,#7c3aed,#2563eb)', borderRadius:99, transition:'width 0.4s ease' }}/></div>
        {nivel.tiempoBonus && (<div style={{ padding:'4px 14px', borderRadius:20, background: tiempo < 20 ? '#fee2e2' : '#f0fdf4', color: tiempo < 20 ? '#dc2626' : '#16a34a', fontWeight:800, fontSize:15, minWidth:70, textAlign:'center' }}>⏱ {tiempo}s</div>)}
      </div>
      ...
      <div style={sv.mainCard}>
        {esEscribir ? (
          <div style={sv.letraDisplay}>
            ...
            <div style={{ color:'#888', fontSize:14, marginTop:4 }}>{pregunta.tipo === 'completar' ? `Completa la matriz para la letra ${pregunta.letra}` : `Escribe la letra ${pregunta.letra} en braille`}</div>
          </div>
        ) : (
        ...
        <div style={{ display:'flex', justifyContent:'center', alignItems:'center', gap:40, marginBottom:24 }}>
          <MatrizBraille matriz={matriz} correcta={correcta} estado={estado} onToggle={esEscribir ? togglePunto : null} teclaPulsada={teclaPulsada} mostrarPista={mostrarPista} invidente={false} />
        ...
        {esLeer && opcionesActuales.length > 0 && (
          <div style={{ display:'grid', gridTemplateColumns:`repeat(${Math.min(opcionesActuales.length, 4)}, 1fr)`, gap:10, marginBottom:16 }}>
            {opcionesActuales.map(op => {
              ...
              const bg = sel ? estado === 'correcto' ? '#f0fdf4' : estado === 'incorrecto' ? '#fef2f2' : '#f5f3ff' : '#f9fafb'
              const border = sel ? estado === 'correcto' ? '#22c55e' : estado === 'incorrecto' ? '#ef4444' : '#7c3aed' : '#e5e7eb'
              return (
                <button key={op} style={{
                    padding:'16px 8px', borderRadius:12, fontWeight:800, fontSize:28, border:`2px solid ${border}`,
                    background:bg, cursor:'pointer', transition:'all 0.15s',
                    transform: sel ? 'scale(1.05)' : 'scale(1)', boxShadow: sel ? `0 4px 12px ${border}44` : 'none',
        ...
        <div style={sv.botones}>
          {esEscribir && (
            <>
              ...
              {!nivel.pistaAutomatica && (<button style={{ ...sv.btnSecundario, background: mostrarPista ? '#ede9fe' : '#f5f5f5' }} onClick={() => setMostrarPista(!mostrarPista)}>💡 Pista</button>)}
            </>
          )}
      ...
      <div style={sv.statsBar}>
        {[{ v:aciertos, l:'Aciertos', c:'#22c55e' }, { v:intentos, l:'Intentos', c:'#ea580c' }, { v:Math.round((aciertos/Math.max(intentos,1))*100)+'%', l:'Precisión', c:'#7c3aed' }, { v:secuencia.length - indiceActual, l:'Restantes', c:'#2563eb' }].map((s,i) => (
          <div key={i} style={sv.statItem}><span style={{ fontSize:20, fontWeight:800, color:s.c }}>{s.v}</span><span style={{ fontSize:12, color:'#888' }}>{s.l}</span></div>
        ))}
      </div>
    </div>
  )
